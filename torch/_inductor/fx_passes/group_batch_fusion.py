...
class PostGradBatchLinearFusion(BatchFusion):
    ...
    def match(self, node: torch.fx.Node) -> Optional[Tuple[str, int, int, int, bool]]:
        ...
        elif CallFunctionVarArgs(aten.addmm.default).match(node) and \
            self._addmm_node_can_be_fused(node):
...
def group_batch_fusion_passes(graph: torch.fx.Graph, pre_grad=True):
    ...
    if pre_grad:
        ...
        fbgemm_fusion_keys = [x for x in config.post_grad_fusion_options \
            if config.post_grad_fusion_options[x].get('require_fbgemm', False)]
        fbgemm_fusions = { fusion: config.post_grad_fusion_options[fusion] \
            for fusion in fbgemm_fusion_keys }
        non_fbgemm_fusions = { fusion: config.post_grad_fusion_options[fusion] \
            for fusion in config.post_grad_fusion_options.keys() \
            if not fusion in fbgemm_fusion_keys }
